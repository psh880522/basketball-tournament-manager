...
export async function listMatchesForResultEntry(
  ...
  let query = supabase
    .from("matches")
    .select(
      "id,tournament_id,division_id,group_id,team_a_id,team_b_id,scheduled_at,court_id,status,score_a,score_b,created_at,divisions(name),groups(name),team_a:teams!matches_team_a_id_fkey(team_name),team_b:teams!matches_team_b_id_fkey(team_name),court:courts(name)"
    )
    .eq("tournament_id", tournamentId);
...
export async function listTournamentMatches(
  ...
  let query = supabase
    .from("matches")
    .select(
      "id,tournament_id,division_id,group_id,scheduled_at,court_id,status,score_a,score_b,created_at,divisions(name),groups(name),team_a:teams!matches_team_a_id_fkey(team_name),team_b:teams!matches_team_b_id_fkey(team_name),court:courts(name)"
    )
    .eq("tournament_id", tournamentId);
