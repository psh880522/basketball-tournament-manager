# 구현 계획: Admin Tournament Result UI & Seeding Update (v2)
...
- **토너먼트 진행 상태 섹션 제거**
  ...
  - 데이터 페칭(`getTournamentBracketProgress`)은 UI에서 사용하지 않으므로, 필요 시 제거 여부만 판단
...
#### 결정 사항 (메모 반영)
- **선택**: *팀 배치 시 `groups.order` 기반으로 첫 라운드를 선택*하는 방식으로 변경
- **이유**
  - 실제 생성된 토너먼트 라운드 구조가 가장 신뢰 가능한 소스
  - `division.tournament_size`는 유지하되, 배치 로직의 단일 기준으로 사용하지 않음
  - 라운드 생성/정렬이 groups에 이미 반영되어 있어 추가 업데이트가 불필요

> 참고: “토너먼트 경기 생성 시 입력된 값을 division.tournament_size에 업데이트” 방식은
> **보조 메타데이터 유지 목적**에는 유용하나, **실제 배치 기준**으로는 취약함.
...
> `size`는 standings limit(배치 대상 팀 수) 계산에만 활용하고,
> **첫 라운드 결정에는 사용하지 않음**.
