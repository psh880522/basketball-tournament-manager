# Plan: 토너먼트 경기 순서 및 스케줄 배치 정비 (v2)
...
### 5) schedule.ts(legacy) 처리 방향 확정
- 메모 반영: schedule.ts 레거시 경로 제거/정리 필요.
- 변경 방향(계획 단계):
  - 현재 운영 플로우에서 schedule.ts가 사용되는지 먼저 확인.
  - 사용되지 않는다면 제거 또는 경로 고정(호출부 제거)까지 포함해 정리.
  - 사용 중이라면 동일한 정렬 기준을 적용하여 동작을 맞춘 뒤, 추후 제거로 전환.
...
- lib/api/schedule.ts (레거시 경로 사용 여부에 따라 수정/정리)
...
### H) lib/api/schedule.ts (레거시 경로)
```ts
// 사용 중이면 정렬 기준을 sortTournamentMatches로 통일
// 사용하지 않으면 호출부 제거 후 파일 정리
```
